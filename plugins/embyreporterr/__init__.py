...
class EmbyReporterr(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._emby_xjj_path = config.get("emby_xjj_path")
    ...
    def __update_config(self):
        self.update_config({
            ...
            "emby_xjj_path": self._emby_xjj_path,
    ...
    def get_report(self, days, types=None, user_id=None, end_date=datetime.now(pytz.timezone("Asia/Shanghai")),
        ...
        for item in result:
            item_id = item[1]
            if not self.is_xjj(item_id):
                filter_result.append(item)
        return True, filter_result

    def is_xjj(self, item_id):
        if not self._emby_xjj_path:
            return False
        xjj_paths = self._emby_xjj_path.split(',')
        api_url = f"{self.host}/emby/Items"
        param = {
            "api_key": self.api_key,
            "ids": item_id,
            'Fields': 'Path'
        }
        result = RequestUtils().get_res(api_url, params=param)
        movies = result.json()['Items']
        if movies:
            movie = movies[0]
            for xjj_path in xjj_paths:
                if xjj_path in movie['Path']:
                    return True
        return False
